...
from ur_robotiq_app.msg import PickAndPlaceAction, PickAndPlaceResult
...
class pickAndPlace(object):
	...
	def __init__(self):
		...
		self._gazebo = rospy.get_param('~gazebo', False)

		#robotiq initialization
		if self._gazebo:
			self._robotiq_client = actionlib.SimpleActionClient('/robotiq_2f_85_position_controller/gripper_cmd', GripperCommandAction)
			self._robotiq_client.wait_for_server()
			rospy.loginfo('confirmed gripper action server started')
		else:
		...
		#camera
		self._use_camera = rospy.get_param('~camera', False)
	...
	def wait_ar_marker(self, marker_name):
		...
		if self._use_camera:
			...
			return True, [-0.65, 0.0, 0.9], [0,0,1.5707] #dummy marker position
